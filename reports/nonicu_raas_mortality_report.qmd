...
![End-to-end cohort construction and analysis workflow.](../assets/workflow_diagram.svg){fig-alt="Analysis workflow diagram" width="88%"}
...
Source notebooks: [cohort construction](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/01_cohort.ipynb) and [exposure definition](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/02_exposure.ipynb).
...
Source notebook and notes: [baseline cohort description](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/03b_describe_analysis_dataset.ipynb) and [short documentation](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/docs/03b_describe_analysis_dataset_SHORT.md).

![Baseline cohort size, age, and hospital length of stay by early RAAS exposure.](../assets/baseline_characteristics_summary.png){fig-alt="Baseline characteristics summary by early RAAS exposure" width="88%"}
...
The unadjusted outcome table is derived from `python/outputs/python_unadjusted_outcomes.csv`. These estimates are descriptive only and do not address baseline differences between exposure groups.

Source notebook and notes: [unadjusted outcomes](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/04a_unadjusted_outcomes.ipynb) and [short documentation](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/docs/04a_unadjusted_outcomes_SHORT.md).
...
![Observed in-hospital mortality proportion by early RAAS exposure group.](../assets/unadjusted_mortality_by_raas.png){fig-alt="Bar chart comparing unadjusted in-hospital mortality by early RAAS exposure" width="78%"}
...
Source notebook and notes: [multivariable outcomes](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/04b_multivariable_outcomes.ipynb) and [short documentation](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/docs/04b_multivariable_outcomes_SHORT.md).

The selected coefficient table is derived from `python/outputs/python_logistic_parameters.csv`. The table emphasizes the exposure term and representative adjusted covariates; it is not a causal effect table.
...
![Adjusted odds ratios from the multivariable model.](../assets/adjusted_odds_ratio_plot.png){fig-alt="Forest plot of adjusted odds ratios from the multivariable model" width="82%"}
...
Source notebook: [multivariable outcomes](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/04b_multivariable_outcomes.ipynb).
...
![Adjusted predicted in-hospital mortality by age and early RAAS exposure status. Predicted mortality increased with age in both groups, while the early RAAS exposure group had lower adjusted predicted mortality across the plotted age range.](../assets/predicted_mortality_by_age.png){fig-alt="Adjusted predicted mortality curves by age and early RAAS exposure" width="78%"}
...
Source notebook and notes: [SAS-Python validation](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/notebooks/05_sas_python_validation.ipynb), [short documentation](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/docs/05_sas_python_validation_SHORT.md), and [validation notes](https://github.com/makotoy56/mimic-iv-nonicu-medication/blob/main/docs/VALIDATION_NOTES.md).

The table below uses existing exported files in `python/outputs/` and `sas/outputs/`.
...
![SAS-Python validation summary for selected aggregate outputs.](../assets/sas_python_validation_summary.png){fig-alt="SAS-Python validation summary plot" width="78%"}
...
Reproducibility is implemented through version-controlled SQL definitions, Python analysis notebooks, SAS validation programs, aggregate CSV outputs, and exported figures. Those artifacts support the tables and figures shown in this report without exposing patient-level data.
